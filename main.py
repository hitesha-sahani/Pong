...
paddle_a.color("Green")  # giving color to paddle
...
ball.dx=0.3  # It moves towards the positive x by 0.1 pixels
ball.dy=-0.3  # The ball moves towards the positive y by 0.1 pixels (i.e. the ball moves diagonally with a positive slope)
...
wn.onkeypress(paddle_a_up, "u")  # When the user presses the key "w", call the function paddle_a_up()
wn.onkeypress(paddle_a_down, "d") # When the user presses the key "s",call the function paddle_a_down()
...
while not game_over:
    ...
    if ball.ycor()>290:  # If y-co-ordinate of ball is greater than 290 pixels
        ...
        winsound.PlaySound('bounce.wav',winsound.SND_ASYNC) # Every time the ball hits the wall it plays the sound
    ...
    if ((ball.xcor() > 340 and ball.xcor() < 350) and (ball.ycor() < (paddle_b.ycor() + 40) and ball.ycor() > (paddle_b.ycor() - 40))):
        ...
        ball.dx *= -1
        ball.setx(340)
        winsound.PlaySound('bounce.wav',winsound.SND_ASYNC)# It plays the sound when the ball hits the paddle
    ...
    if score_a == 3:
        ...
        pen.showturtle()
        try:
            pen.write("Player A WON!!!", align="center", font=("Courier", 20, "normal"))
